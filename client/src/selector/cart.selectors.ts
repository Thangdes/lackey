...
export function selectSmartCartItems(serverItems: ServerCartItem[]): SmartCartItem[] {
  ...
  return items.map((it) => {
    // Debug log to check if slug is available
    if (process.env.NODE_ENV === 'development') {
      console.log('[CART_SELECTOR] Product data:', {
        productId: it?.product?.id,
        productSlug: it?.product?.slug,
        variantProductId: it?.productVariant?.product?.id,
        variantProductSlug: it?.productVariant?.product?.slug,
      });
    }
    return {
    itemId: it.id,
